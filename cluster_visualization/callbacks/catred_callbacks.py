...
class CATREDCallbacks:
    ...
    def _setup_manual_catred_render_callback(self):
        ...
        def manual_render_catred_data(
            ...
            try:
                ...
                # Status info
                total_catred_traces = len(existing_catred_traces) + (
                    1 if catred_scatter_data and catred_scatter_data["ra"] else 0
                )
                catred_points_count = (
                    len(catred_scatter_data["ra"])
                    if catred_scatter_data and catred_scatter_data["ra"]
                    else 0
                )
                ...
                status = dbc.Alert(
                    [
                        ...
                        html.Small(
                            f"CATRED data rendered at: {pd.Timestamp.now().strftime('%H:%M:%S')}",
    ...
    def load_catred_scatter_data(
        self, data, relayout_data, catred_masked=False, threshold=0.8, maglim=None
    ):
        """Load CATRED scatter data using modular or fallback method"""
        if self.catred_handler:
            return self.catred_handler.load_catred_scatter_data(
                data, relayout_data, catred_masked, threshold, maglim
            )
        else:
